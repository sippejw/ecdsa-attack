...
func getCipherSuitesMap() map[uint16]string {
...
func getServerPref(server string, serverName string, cipherSuites []uint16) (uint16, error) {
	...
	// Set cipher suites
	tlsConn.HandshakeState.Hello.CipherSuites = cipherSuites
	...
	if err != nil {
		if err.Error() != "tls: server chose an unconfigured cipher suite" {
			return 0, err
		}
	}

	return tlsConn.HandshakeState.ServerHello.CipherSuite, nil
}

func RemoveValue(s []uint16, value uint16) []uint16 {
	var index int
	for i := 0; i < len(s); i++ {
		if s[i] == value {
			index = i
...
func getCipherSuites() []uint16 {
	csm := getCipherSuitesMap()
	keys := make([]uint16, 0, len(csm))
	for k := range csm {
		keys = append(keys, k)
	}
	return keys
...
func getPreferedList(server string, serverName string) ([]uint16, error) {
	/*
		cipherSuites := []uint16{
	...
	for len(cipherSuites) > 0 {
		...
		//fmt.Printf("Trying with list len %d\n", len(cipherSuites))
		cs, err := getServerPref(server, serverName, cipherSuites)
		if err != nil {
		...
		//fmt.Printf("got %x\n", cs)
		prefList = append(prefList, cs)
		cipherSuites = RemoveValue(cipherSuites, cs)
	}
...
func strPrefList(prefList []uint16) []string {
	csm := getCipherSuitesMap()

	var out []string

	for _, cs := range prefList {
		out = append(out, fmt.Sprintf("%s (%04x)", csm[cs], cs))
	}
	return out
}
...
func main() {
	...
	server := os.Args[1]
	//list, err := getPreferedList("220.181.111.91:443", "pan.baidu.com")
	//list, err := getPreferedList("54.174.72.166:443", "ericw.us")

	list, err := getPreferedList(fmt.Sprintf("%s:443", server), server)
	if err != nil {
		fmt.Printf("err: %+v\n", err)
		return
	}

	fmt.Printf("%d preferred ciphers:\n", len(list))
	for _, s := range strPrefList(list) {
		fmt.Printf("%s\n", s)
	}
